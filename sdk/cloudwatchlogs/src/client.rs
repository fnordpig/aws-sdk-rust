...
pub mod fluent_builders {
    ...
    /// <p>Exporting log data to Amazon S3 buckets that are encrypted by KMS is supported. Exporting log data to Amazon S3 buckets that have S3 Object Lock enabled with a retention period is also supported.</p> 
    ...
    #[deprecated(note = "Please use the generic tagging API ListTagsForResource")]#[derive(std::clone::Clone, std::fmt::Debug, )]
    ...
    /// Fluent builder constructing a request to `PutRetentionPolicy`.
                        ///
    /// <p>Sets the retention of the specified log group. A retention policy allows you to configure the number of days for which to retain log events in the specified log group.</p> <note> 
    ...
    /// <p>This means that if you change a log group to have a longer retention setting when it contains log events that are past the expiration date, but haven’t been actually deleted, those log events will take up to 72 hours to be deleted after the new retention date is reached. To make sure that log data is deleted permanently, keep a log group at its lower retention setting until 72 hours has passed after the end of the previous retention period, or you have confirmed that the older log events are deleted. </p> 
    /// </note>
    #[derive(std::clone::Clone, std::fmt::Debug, )]
    ...
    /// <p> You are limited to 20 concurrent CloudWatch Logs insights queries, including queries that have been added to dashboards. </p>
    #[derive(std::clone::Clone, std::fmt::Debug, )]
    ...
    /// <p>CloudWatch Logs doesn’t support IAM policies that prevent users from assigning specified tags to log groups using the <code>aws:Resource/<i>key-name</i> </code> or <code>aws:TagKeys</code> condition keys. For more information about using tags to control access, see <a href="https://docs.aws.amazon.com/IAM/latest/UserGuide/access_tags.html">Controlling access to Amazon Web Services resources using tags</a>.</p>
    #[deprecated(note = "Please use the generic tagging API TagResource")]#[derive(std::clone::Clone, std::fmt::Debug, )]
    ...
    #[deprecated(note = "Please use the generic tagging API UntagResource")]#[derive(std::clone::Clone, std::fmt::Debug, )]
